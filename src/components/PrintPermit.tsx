            ...
            <div className="flex flex-col h-full text-black leading-snug">
              ...
              <div className="space-y-1.5 pb-2 mb-3 text-black" style={{ fontSize: '14.5pt' }}>
                ...
                <div className="flex items-baseline">
                  ...
                  <span className="ml-2 font-bold border-b border-dotted border-slate-400/80 flex-grow pl-1 pb-0 text-black">
              ...
              <div className="space-y-2 text-justify flex-grow" style={{ fontSize: '14.5pt', lineHeight: '1.25' }}>
                <p className="leading-relaxed text-justify" style={{ textIndent: '2.5em', marginBottom: '17px', paddingLeft: '0px', lineHeight: '36px', height: 'auto', textAlign: 'justify', textDecoration: 'none' }}>
                  ด้วยข้าพเจ้า <span className="font-bold px-1" style={{ textDecoration: 'underline dotted rgba(148, 163, 184, 0.8)', textUnderlineOffset: '2px' }}>{booking.requesterName}</span> ตำแหน่ง <span className="font-bold px-1" style={{ textDecoration: 'underline dotted rgba(148, 163, 184, 0.8)', textUnderlineOffset: '2px' }}>{booking.requesterPosition}</span> สังกัดกลุ่มงาน <span className="font-semibold px-1" style={{ textDecoration: 'underline dotted rgba(148, 163, 184, 0.8)', textUnderlineOffset: '2px' }}>{booking.department}</span> มีความประสงค์ขอใช้รถยนต์ส่วนกลาง/ราชการ เพื่อเดินทางไปปฏิบัติราชการพื้นที่ <span className="font-bold px-1" style={{ textDecoration: 'underline dotted rgba(148, 163, 184, 0.8)', textUnderlineOffset: '2px' }}>{booking.destination}</span> เพื่อ <span className="font-medium px-1" style={{ textDecoration: 'underline dotted rgba(148, 163, 184, 0.8)', textUnderlineOffset: '2px' }}>{booking.purpose}</span> โดยมีคนนั่งจำนวน <span className="font-bold px-1" style={{ textDecoration: 'underline dotted rgba(148, 163, 184, 0.8)', textUnderlineOffset: '2px' }}>{toThaiNumerals(booking.passengersCount?.toString())}</span> คน คือ <span className="font-semibold px-1" style={{ textDecoration: 'underline dotted rgba(148, 163, 184, 0.8)', textUnderlineOffset: '2px' }}>{booking.passengersList && booking.passengersList.filter(n => n.trim() !== '').length > 0 ? `ข้าพเจ้าและผู้ร่วมทาง ได้แก่ ${booking.passengersList.filter(n => n.trim() !== '').map((n, idx) => {
                    const pos = booking.passengersPositionsList?.[idx];
                    return `${n}${pos ? ` (${pos})` : ''}`;
                  }).join(', ')}` : 'ข้าพเจ้าลำพัง'}</span> ในวันที่ <span className="font-bold px-1" style={{ textDecoration: 'underline dotted rgba(148, 163, 184, 0.8)', textUnderlineOffset: '2px' }}>{toThaiNumerals(startDateThaiOnly)}</span> เวลา <span className="font-bold px-1" style={{ textDecoration: 'underline dotted rgba(148, 163, 184, 0.8)', textUnderlineOffset: '2px' }}>{toThaiNumerals(formatPrintTime(booking.startDate))} น.</span> {booking.endDate ? (
                    <>
                      ถึงวันที่ <span className="font-bold px-1" style={{ textDecoration: 'underline dotted rgba(148, 163, 184, 0.8)', textUnderlineOffset: '2px' }}>{toThaiNumerals(endDateThaiOnly)}</span> เวลา <span className="font-bold px-1" style={{ textDecoration: 'underline dotted rgba(148, 163, 184, 0.8)', textUnderlineOffset: '2px' }}>{toThaiNumerals(formatPrintTime(booking.endDate))} น.</span>
                    </>
                  ) : 'เป็นต้นไป'}
                </p>
                ...
                <div className="pt-2 flex justify-end text-center">
                  <div className="w-80 leading-normal" style={{ fontSize: '14.5pt' }}>
                    <p className="flex items-baseline justify-center">
                      ...
                      <span className="w-[140px] overflow-hidden whitespace-nowrap text-center text-slate-400 select-none" style={{ position: 'relative', top: '1.5px' }}>......................................................................</span>
                      <span className="w-24 text-left pl-1 shrink-0 font-sans">ผู้ขออนุญาต</span>
                    </p>
                    <div className="font-bold text-black" style={{ lineHeight: '15.2333px', marginTop: '4px' }}>
                      ( {booking.requesterName} )
                      <br />
                      <span className="text-[14.5pt] text-slate-600 font-semibold" style={{ marginTop: '0px', marginBottom: '0px', paddingTop: '0px', paddingBottom: '0px' }}>ตำแหน่ง: {booking.requesterPosition}</span>
                    </div>
                  </div>
                </div>

                {/* Recommendations opinion row (เห็นควรอนุญาต) */}
                <div className="space-y-1.5 mt-3 pl-2 pb-3.5" style={{ lineHeight: '19.2333px' }}>
                ...
                <div className="border border-black rounded-lg p-3.5 mt-3 supplementary-box relative bg-transparent flex flex-col justify-between" style={{ fontSize: '14.5pt' }}>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-3 mb-2">
                    ...
                    <div className="text-center leading-normal">
                      <p className="flex items-baseline justify-center">
                        ...
                        <span className="w-[143px] overflow-hidden whitespace-nowrap text-center text-slate-400 select-none" style={{ position: 'relative', top: '1.5px' }}>......................................................................</span>
                        <span className="text-left shrink-0 font-sans text-[14.5pt]" style={{ width: '101px', marginLeft: '1px', paddingLeft: '4px', paddingRight: '0px', paddingTop: '0px', paddingBottom: '0px' }}>ผู้ควบคุมใช้รถ</span>
                      </p>
                      <p className="font-bold mt-1 text-black" style={{ fontSize: '14.5pt', paddingRight: '48px' }}>
                        ( {booking.caretakerName || '..........................................................'} )
                      </p>
                      <p className="text-[14.5pt] text-slate-500" style={{ marginTop: '-3px', paddingRight: '41px', paddingTop: '0px' }}>
                        {booking.caretakerPosition || 'นักวิชาการจัดดูแลยานพาหนะ'}
                      </p>
                    </div>
                  ...
                  <div className="pt-3 text-center leading-normal max-w-sm mx-auto mt-1">
                    <p className="flex items-baseline justify-center" style={{ paddingRight: '0px', paddingLeft: '57px' }}>
                      <span className="w-12 text-right pr-1 shrink-0 select-none">ลงชื่อ</span>
                      <span className="overflow-hidden whitespace-nowrap text-center text-slate-400 select-none" style={{ width: '161px', position: 'relative', top: '1.5px' }}>......................................................................</span>
                      <span className="text-left shrink-0 font-sans" style={{ paddingLeft: '5px', width: '116.9896px' }}>ผู้มีอำนาจสั่งใช้รถ</span>
                    </p>
                    <p className="font-bold text-black" style={{ fontSize: '14.5pt', marginTop: '3px' }}>
                      ( {booking.approvedBy} )
                    </p>
                    <p className="text-[14.5pt] text-slate-600 font-bold leading-tight" style={{ lineHeight: '20.2333px', marginTop: '-1px' }}>
